...
export default function ExerciseHistoryScreen() {
  ...
  const loadHistory = async () => {
    try {
      ...
      // Invoke exercise progression Edge Function
      const { data, error } = await supabase.functions.invoke('get-client-exercise-history', {
        body: { clientId: session?.user?.id, exerciseId: id, limit: 10 }
      });
