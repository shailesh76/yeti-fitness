...
- **`profiles`** — one row per `auth.users` row (auto-created by a trigger on signup, see
  ...
  `athlete_id` — see audit finding #2 for a regression that must never be reintroduced here). Phase A
  protects `role` with `prevent_untrusted_profile_role_change`; ordinary self-profile updates remain
  available, but role changes require an authorized server process.
...
> `20260910120000_exercise_media_primary.sql` is the pending C2C migration. It permits zero or one
> primary row per exercise, enforces that invariant with a partial unique index, and exposes an atomic
> service-role-only primary-selection function behind the authorized media Edge Function. Primary
> selection is independent of `media_status` and never publishes an asset.
...
- **`exercise_alternatives`** — direct exercise substitutes (`exercise_id`, `alternative_exercise_id`, `reason`, `created_at`). Direct mutations revoked; managed atomically via `save_exercise_editor_v2`.
- **`exercise_progressions`** — step-up progression exercises (`exercise_id`, `progression_exercise_id`, `difficulty_delta`, `created_at`). Direct mutations revoked; managed atomically via `save_exercise_editor_v2`.
- **`exercise_regressions`** — step-down regression exercises (`exercise_id`, `regression_exercise_id`, `difficulty_delta`, `created_at`). Direct mutations revoked; managed atomically via `save_exercise_editor_v2`.
- **`exercise_relations`** — self-referential `(exercise_id, related_exercise_id, relation_type)` legacy lookup where `relation_type` is `variation` or `alternative`. Deprecated and superseded by dedicated Phase 1/Phase B relation tables.
