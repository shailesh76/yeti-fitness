...
export default function ProgressScreen() {
  ...
  const loadWeightLogs = async () => {
    ...
    try {
      const data = await progressRepository.getWeightTrend(session.user.id);
      const logs = data
  ...
  const loadLatestInsight = async () => {
    if (!session?.user?.id) return;
    try {
      const data = await progressRepository.fetchInsightsRemote(session.user.id);
  ...
  const loadMeasurements = async () => {
    if (!session?.user?.id) return;
    try {
      const data = await progressRepository.getMeasurements(session.user.id);
      const map: typeof measurements = {};
      MEASUREMENT_TYPES.forEach(({ key }) => {
        const fieldName = key === 'thighs' ? 'legs_cm' : `${key}_cm`;
        const rows = data.filter((m: any) => (m as any)[fieldName] !== undefined && (m as any)[fieldName] !== null);
        map[key] = {
          current: rows[0] ? (rows[0] as any)[fieldName] : null,
          prev:    rows[1] ? (rows[1] as any)[fieldName] : null,
        };
      });
      setMeasurements(map);
    } catch (e) {
  ...
  const handleSaveMeasurement = async (key: string) => {
    ...
    try {
      ...
      const chest = key === 'chest' ? val : undefined;
      const waist = key === 'waist' ? val : undefined;
      const hips = key === 'hips' ? val : undefined;
      const arms = key === 'arms' ? val : undefined;
      const legs = key === 'thighs' ? val : undefined;

      await progressRepository.saveMeasurement(
        session!.user!.id,
        weight,
        undefined,
        chest,
  ...
  const loadProgressPhotos = async () => {
    if (!session?.user?.id) return;
    try {
      const data = await mediaRepository.getPhotoMetadata(session.user.id);
      if (data) {
        const resolved = await Promise.all(
          data.map(async (p: any) => {
            const url = await getSignedUrl(p.photo_key);
            return { id: p.id, uri: url, date: new Date(p.logged_at).toISOString(), key: p.photo_key };
          })
        );
  ...
  const handleAddProgressPhoto = async () => {
    try {
      ...
      await mediaRepository.savePhotoMetadata(session!.user!.id, uploadRes.key, '');
  ...
  const handleDeleteProgressPhoto = async (id: string, photoKey: string) => {
    const performDelete = async () => {
      try {
        await mediaRepository.deletePhotoRemote(id, photoKey);
        await loadProgressPhotos();
      } catch (e) {
  ...
  const handleAddWeight = async () => {
    ...
    try {
      await progressRepository.saveMeasurement(session!.user!.id, val);
      setNewWeight('');
      loadWeightLogs();
    } catch (e) {
